  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50">
        <nav
          className={`container mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between transition-all duration-300 ease-in-out
          ${scrolled
        ...
        >
          ...
          {/* Search Bar */}
          <form onSubmit={handleSearch} className="hidden md:flex flex-none w-64 justify-end ml-auto">
          ...
          {/* Mobile Hamburger */}
          <button
            className="text-white md:hidden ml-2 w-10 h-10 rounded-lg backdrop-blur-sm bg-white/10 border border-white/10 hover:bg-white/20 flex items-center justify-center transition-all duration-200"
        ...
        {isOpen && (
          <div className="md:hidden fixed inset-x-0 top-16 mt-2 mx-4 rounded-2xl shadow-2xl border border-white/20 backdrop-blur-xl bg-white/15 animate-slide-down">
            <div className="flex flex-col items-center py-6 space-y-4">
              <Link to="/" className="text-white/90 hover:text-white px-6 py-3 rounded-full w-4/5 text-center backdrop-blur-sm bg-white/10 border border-white/10 hover:bg-white/20 transition-all" onClick={() => setIsOpen(false)}>Home</Link>
              <Link to="/lessons" className="text-white/90 hover:text-white px-6 py-3 rounded-full w-4/5 text-center backdrop-blur-sm bg-white/10 border border-white/10 hover:bg-white/20 transition-all" onClick={() => setIsOpen(false)}>Explore</Link>
              <Link to="/contact" className="text-white/90 hover:text-white px-6 py-3 rounded-full w-4/5 text-center backdrop-blur-sm bg-white/10 border border-white/10 hover:bg-white/20 transition-all" onClick={() => setIsOpen(false)}>Contact</Link>
            </div>
          </div>
        )}
      </header>
